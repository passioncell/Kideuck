...
public class PostingRepository implements IPostingRepository {
    ...
    @Override
    public DetailPostingModel getDetailPosting(int postingId, int userId) {
        try {
            ...
            Obj = (org.json.JSONObject) array.get(0);
            detailPostingModel = new DetailPostingModel(
                    ...
                    Integer.parseInt(Obj.get("imageCnt").toString())
            );

            return detailPostingModel;
...
//            }
    ...
    //(신)
    @Override
    public boolean writePost(int userId, String content, boolean isImage) {
        try {
    ...
    @Override
    public int getImageCnt(int postingId) {

        try {
            url = new URL(URLInfo.Posting_GetImgCnt);
            httpMessage = new HttpMessage(url);
            Properties prop = new Properties();
            prop.setProperty("postingId", String.valueOf(postingId));
